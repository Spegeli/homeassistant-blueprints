...
| 7 | Room humidity < turn-off threshold (50 %) and minimum run time (15 min) reached | off |
...
The "please ventilate" actions run once when the recommendation starts. After a restart or a reload of the automations the 30-minute timer cannot continue, so the dehumidifier takes over right away if the humidity is above the target.
...
**Device hygrostat:** many dehumidifiers measure humidity in their own, already dried air stream and stop too early. With "Override the Device's Hygrostat" the automation sets the device's target humidity to its minimum whenever it turns the device on, so only the room sensor decides when it stops.
...
Values of 70 % and above are shown in bold. Room air at 55 % already reaches 80 % at a 14 °C wall. Basement walls in contact with the ground are often that cold, which is why the defaults (target 55 %, off 50 %, critical 60 %) are stricter than the general 40–60 % recommendation.
...
- **Drying deeper costs more:** the drier the air, the less water each kWh removes. In a US simulation, a 50 % setpoint used about five times the energy of 60 %. Don't set the turn-off threshold lower than necessary.
...
- **Settings:** "defaults" means target 55 %, off 50 %, critical 60 %, minimum drying effect 6 points, comfort 20 °C, minimum room temperature 18 °C, wait time 30 min, minimum run time 15 min, minimum off time 5 min. Changed values are bold.
...
| 8 | 20 °C / 62 % | 10.7 g/m³ / 12.5 °C | 2 °C / 90 % | 5.0 g/m³ / 0.5 °C | +12.0 K | 27 % | +35 pts | defaults | – | Dehumidifier **on** – critical humidity (≥ 60 %); ventilation is recommended as well |
